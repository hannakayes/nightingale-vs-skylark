...
// Add constants for the villain images
const eagle_left = document.getElementById('eagle_left');
const eagle_right = document.getElementById('eagle_right');
const crow_left = document.getElementById('crow_left');
const crow_right = document.getElementById('crow_right');

let villainInterval = null; // Variable to store interval for villain appearance
let currentVillain = null; // Variable to store the current villain

function startVillainAppearance() {
    let villainAppearedForLevel = false; // Flag to track if a villain has appeared for the current level

    villainInterval = setInterval(() => {
        if (!currentVillain && gameStats.level >= 5 && !villainAppearedForLevel) {
            const randomIndex = Math.floor(Math.random() * 4); // Random index from 0 to 3
            let selectedVillain;

            switch (randomIndex) {
                case 0:
                    selectedVillain = eagle_left;
                    selectedVillain.style.display = 'block';
                    initializeVillainPosition(selectedVillain);
                    break;
                case 1:
                    selectedVillain = eagle_right;
                    selectedVillain.style.display = 'block';
                    initializeVillainPosition(selectedVillain);
                    break;
                case 2:
                    selectedVillain = crow_left;
                    selectedVillain.style.display = 'block';
                    initializeVillainPosition(selectedVillain);
                    break;
                case 3:
                    selectedVillain = crow_right;
                    selectedVillain.style.display = 'block';
                    initializeVillainPosition(selectedVillain);
                    break;
                default:
            ...
            currentVillain = selectedVillain;
            villainAppearedForLevel = true; // Set flag indicating a villain has appeared for this level
        }
    }, calculateVillainInterval()); // Initial interval based on current level

    // Function to calculate interval dynamically based on level
    function calculateVillainInterval() {
        const baseInterval = 5000; // Base interval in milliseconds
        const intervalReductionPerLevel = 300; // Reduction per level in milliseconds

        // Calculate interval based on current level
        const currentInterval = baseInterval - (intervalReductionPerLevel * (gameStats.level - 5));
        return Math.max(currentInterval, 1000); // Ensure interval never goes below 1000 milliseconds
    }

    // Reset villain appearance flag when level increases
    gameStats.incrementLevel = function() {
        this.level++;
        villainAppearedForLevel = false; // Reset flag when level increases
        if (this.level >= 5) {
            if (!villainInterval) {
                startVillainAppearance();
            }
...
function initializeVillainPosition(villain) {
    const gameAreaWidth = gameArea.clientWidth;
    const villainWidth = villain.clientWidth;
    const initialTop = Math.random() * (gameArea.clientHeight - villain.clientHeight);

    if (villain === crow_left) {
        villain.style.left = '-' + villainWidth + 'px'; // Start from the left side
        villain.style.top = initialTop + 'px';
    } else if (villain === eagle_right) {
        villain.style.left = gameAreaWidth + 'px'; // Start from the right side
        villain.style.top = initialTop + 'px';
    } else if (villain === crow_right) {
        villain.style.left = gameAreaWidth + 'px'; // Start from the right side
        villain.style.top = initialTop + 'px';
    } else if (villain === eagle_left) {
        villain.style.left = '-' + villainWidth + 'px'; // Start from the left side
        villain.style.top = initialTop + 'px';
    }
...
function moveVillain(villain) {
    ...
    const moveInterval = setInterval(() => {
        const villainLeft = parseInt(villain.style.left);

        if (villain === crow_left || villain === eagle_left) {
            moveLeft();
        } else if (villain === crow_right || villain === eagle_right) {
            moveRight();
        }
    }, 16); // Adjust interval for smoother movement

    function moveLeft() {
        const villainLeft = parseInt(villain.style.left);
        if (villainLeft < gameArea.clientWidth) {
            villain.style.left = villainLeft + speed + 'px';
        } else {
    ...
    function moveRight() {
        const villainLeft = parseInt(villain.style.left);
        if (villainLeft > -villain.clientWidth) {
            villain.style.left = villainLeft - speed + 'px';
        } else {
            clearInterval(moveInterval);
            villain.style.display = 'none';
